...
extern int errno;
int errnum;
int hash[15000000], m=2, nm,all_kmer, count_of_hash, sg1=1, sg2, final_s1_start[900000], final_s1_end[900000], s2j=0,s2_start[900000], s2_end[900000],l=0,k1, allkmer1, dof,mmk,fgs=0;
double thres=0.9999999999;
...
double a, b, c1, d, beta, neff, arg, sx, smax, sarg, smax1, thres1=0.9999999999999;
...
double p1length, p2length2,new1p1l,thres3=0.9999999999999, numerator, denominator;
...
for(is=1;is<fgs;is++){//foreach segment //is<fgs

	for(i=1; i<fgs-1;i++){// check if part1 already clustered //is<fgs-1
	...
	for(q=secondsegmentcounter;q<fgs; q++){//for second segment     
		...
		if(q<fgs){
			...
			secondsegstart=(q*all_kmer)-all_kmer+1;
    			...
    			for(j=secondsegstart;j<secondsegstart+64;j++){//collect part2 freq
        			part2[ii]=hash1[j];
        			ii+=1;		
                        					     } 

        		for(i=1;i<allkmer1;i++){//add part1 and 2 freq
...
//output4 = fopen("NC_015138_new_weird_mo.coord2", "w");
for(i=1;i<fgs;i++){
...
newparentstart=parent_start+mmk; //replace 255 with formula (4*4*(pow(4,m))-1
